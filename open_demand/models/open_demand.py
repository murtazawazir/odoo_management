...
class SBOpenDemand(models.Model):
    ...
    def write(self, values):
        ...
        for rec in self:
            leader_emails = (
                    rec.owner_team_ids.mapped('user_id.login') +
                    rec.sdm_team_ids.mapped('user_id.login')
            )
            member_emails = (
                    rec.owner_team_ids.mapped('member_ids.email') +
                    rec.sdm_team_ids.mapped('member_ids.email')
            )
            email_to = ','.join(set(leader_emails + member_emails))

            if email_to:
                mail_template = self.env.ref("open_demand.mail_template_send")
                if mail_template:
                    values = {
                        'email_to': email_to,
                    }
                    mail_template.send_mail(
                        rec.id, force_send=True, raise_exception=True, email_values=values
                    )
        return result
